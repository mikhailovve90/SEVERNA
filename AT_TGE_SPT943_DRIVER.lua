	...
	local userArchTypeHour = Core.HourArchiveSelected;		-- тип архива заданный пользователем - часовой
	local userArchTypeDate = Core.DateArchiveSelected;		-- тип архива заданный пользователем - суточный	
	local userArchTypeMonth = Core.MonthArchiveSelected;	-- тип архива заданный пользователем - месячный
	...
	local userStartDate = tostring( Core.StartDate - 10800);				-- СТАЛО
	local userStartDate = tonumber( string.sub(userStartDate, 1, 10));		-- СТАЛО
	Core.addLogMsg( "f1: userStartDate => " .. userStartDate);				-- СТАЛО
	
	local userEndDate = tostring( Core.EndDate - 10800);					-- СТАЛО
	local userEndDate = tonumber( string.sub(userEndDate, 1, 10));			-- СТАЛО
	Core.addLogMsg( "f1: userEndDate => " .. userEndDate);					-- СТАЛО
	
	-- local userStartDate = Core.StartDate - 10800; --РАБОТАЕТ  00:00
	-- local userEndDate = Core.EndDate - 10800; --РАБОТАЕТ  00:00
	
	--	local userStartDate = Core.StartDate; --БЫЛО 03:00
	--local userEndDate = Core.EndDate; --БЫЛО 03:00
...
--------------------------------------------------------------------------------------
		if userArchTypeDate == true then -- задан тип суточного архива
			...
			StartDate = userStartDate;
			endDate = userEndDate;
			Core.SPT943_Driver_Msg = "ЗАПРОС суточного архива ТВ1 с " .. os.date( "%d.%m.%Y %H:%M", StartDate) .. " по " .. os.date( "%d.%m.%Y %H:%M", endDate) .. "...";
			...
			if fdDate_2 == nil then													-- проверка открытия файла суточного архива ТВ2
				Core.SPT943_Driver_Msg = "Ошибка открытия файла суточного архива ТВ2!";
...
--------------------------------------------------------------------------------------
		if userArchTypeMonth == true then -- задан тип месячного архива
			...
			if fdMonth_1 == nil then												-- проверка открытия файла месячного архива ТВ1
				Core.SPT943_Driver_Msg = "Ошибка открытия файла месячного архива ТВ1!";
			...
			if fdMonth_2 == nil then												-- проверка открытия файла месячного архива ТВ2
				Core.SPT943_Driver_Msg = "Ошибка открытия файла месячного архива ТВ2!";
...
function f2()
	if Core.StartRequest == true then
		...
		f1();
		if Core.CancelRequest == true then
...
-- function f2()
	-- if Core.StartRequest == true then
		...
		-- while ( 1) do
			-- ret = f1();
			-- if ret == q or Core.CancelRequest == true then
				-- break;
			-- end
		-- end
		-- if  Core.CancelRequest == true then
...
--Core.onExtChange( {"StartRequest", "CancelRequest"}, f2);
Core.onExtChange( {"StartRequest"}, f2);
Core.waitEvents();