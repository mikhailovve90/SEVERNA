...
local ScreenID="VOS" -- идентификатор мнемосхемы технологического объекта
local e_type -- тип события(появление/исчезновение)
local Logs=true -- вести логи
local USO_ID="SAUVOS_" -- идентификатор УСО
local ObjID="GSP_SEV_KSSEV_E_"..USO_ID -- идентификатор технологического объекта
local PLC_Name="A1_" -- название контроллера в УСО
local PLC_Desc="САУ ВОС. ПЛК A1. " --описание контроллера в УСО
local ObjDesc= "САУ ВОС. "--описание источника сигналов
...
--				reliabilityMsg=ObjDesc..DO_Signals[Ch].Comment.. " ЗНАЧЕНИЕ НЕДОСТОВЕРНО"
		...
		if Value==Faults.no --если неисправностей нет
				then			
					...
					Core.addEvent(reliabilityMsg.." значение НЕДОСТОВЕРНО", event.dr, 0, USO_ID..string.gsub(PLC_Name,"_",""), User,reliabilityField_tag , timestamp, ScreenID) --создадим запись в строке сообщений
				...
				--	Core[reliabilityFlag]=false 
					...
					Core.addEvent(reliabilityMsg.." значение НЕДОСТОВЕРНО", event.dr, 1, USO_ID..string.gsub(PLC_Name,"_",""), User, reliabilityField_tag, timestamp, ScreenID) --создадим запись в строке сообщений
