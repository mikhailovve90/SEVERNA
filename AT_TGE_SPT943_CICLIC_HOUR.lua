	...
	fd = io.open( pathHour_1, "r");								-- открыть файл часового архива ТВ1 на чтение
	...
	local userStartDate = tonumber( ret) + 3600;		
	Core.addLogMsg( "f1: userStartDate => " .. userStartDate);
	
	local userEndDate = userStartDate + 3600;
...
function f2()
	if Core.StartRequestHour == true then
		f1();
	end
end
Core.onExtChange( {"StartRequestHour"}, f2);
Core.waitEvents();